...
<div class="post-container mt-5">
    ...
    <form id="postForm" action="/api/post/reply" method="post" enctype="multipart/form-data">
        <input type="hidden" name="parentPostId" value="${parentPost.postId}"> <%-- parentPost.postId로 변경 --%>
        <div class="row mb-3">
            <div class="col-md-6">
                <label for="authorId" class="form-label">ID</label>
                <input type="text" class="form-control" id="authorId" name="authorId" value="${currentAuthorId}" readonly>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col-md-6">
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" id="postTitle" name="postTitle" value="Re: ${fn:escapeXml(parentPost.postTitle)}" required>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col-md-6">
        ...
        <div class="text-end">
            ...
            <a href="/post/getPostDetail?postId=${parentPost.postId}" class="btn btn-secondary">취소</a> <%-- parentPost.postId로 변경 --%>
        </div>
    </form>
</div>
<script>
    ...
    $(document).ready(function() {
                    ...
                    $.ajax({
                        ...
                        success: (response) => {
                            ...
                            if (response.success && response.data && response.data.url) {
                                callback(response.data.url, "image"); // `response.data.url`로 접근
                            } else {
        ...
        $("#postForm").submit(function(e) {
            ...
            const formData = new FormData();
            formData.append('post', new Blob([JSON.stringify(postData)], { type: 'application/json' })); // JSON 문자열을 Blob으로 변환하여 'post' 파트 생성
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    if (response.success && response.redirectUrl) { // success 필드 확인
                        alert(response.message);
                        window.location.href = response.redirectUrl;
                    } else {
                        alert("답글 작성 실패: " + (response.message || "알 수 없는 오류")); // message 필드 확인
                    }
                },
                error: (xhr) => {
                    console.error("Reply post failed:", xhr.responseJSON);
                    const errorMessage = xhr.responseJSON ? xhr.responseJSON.message : "알 수 없는 오류";
                    alert("답글 작성 중 오류: " + errorMessage);
                }
            });
        });
    });
