...
<body>
    <div class="container-fluid">
        <div class="card shadow-sm">
            <div class="card-header"><h3 class="card-title" style="font-weight: 600;">문의게시판 상세</h3></div>
            <div class="card-body">
                <table class="table table-bordered">
                    <tbody>
                        ...
                        <tr><th style="background-color: #f8f9fa;">조회수</th><td>${post.postReadCount}</td></tr>
                        <tr><th style="background-color: #f8f9fa;">내용</th><td style="min-height: 200px; vertical-align: top;"><div>${post.postContent}</div></td></tr>
                        <c:if test="${not empty post.existingFilePath}">
                            <tr>
                                <th style="background-color: #f8f9fa;">첨부파일</th>
                                <td><a href="/post/download?filePath=${post.existingFilePath}" class="text-decoration-none">${fn:substringAfter(post.existingFilePath, '_')}</a></td>
                            </tr>
                        </c:if>
                    </tbody>
                </table>
            ...
            <div class="card-footer text-end">
                ...
                <c:if test="${not empty currentAuthorId}"><a href="/post/replyPost?postId=${post.postId}" class="btn btn-info">답글</a></c:if>
                <c:if test="${currentAuthorId eq post.authorId}"><a href="/post/updatePost?postId=${post.postId}" class="btn btn-warning">수정</a><a href="/post/deletePost?postId=${post.postId}" class="btn btn-danger ms-1">삭제</a></c:if>
            </div>
        </div>

        <div class="card shadow-sm mt-4">
            <div class="card-header"><h4 class="card-title">댓글 (<span id="commentCount">${fn:length(comments)}</span>)</h4></div>
            <div class="card-body" id="comment-list-container">
                    ...
                    <c:if test="${empty comment.parentCommentId}">
                        <div class="comment-item pt-3 pb-3 border-bottom" id="comment-${comment.commentId}">
                            ...
                            <div class="comment-actions text-end">
                                <c:if test="${not empty currentAuthorId}">
                                ...
                                <c:if test="${currentAuthorId eq comment.writerId}">
                            ...
                            <c:forEach var="reply" items="${comments}">
                                <c:if test="${reply.parentCommentId eq comment.commentId}">
                                    <div class="comment-item reply-comment pt-3 pb-3 mt-3" id="comment-${reply.commentId}">
                                        ...
                                        <div class="comment-actions text-end">
                                            <c:if test="${currentAuthorId eq reply.writerId}">
            ...
            <div class="card-footer">
                <div class="d-flex gap-2 mt-2">
                    <textarea id="newCommentContent" class="form-control" rows="2" placeholder="댓글을 입력하세요" ${empty currentAuthorId ? "disabled" : ""}></textarea>
                    <button type="button" class="btn btn-primary" id="submitCommentButton" style="width: 120px;" ${empty currentAuthorId ? "disabled" : ""}>등록</button>
                </div>
                <div class="invalid-feedback d-block" id="newCommentContent-error"></div>
            </div>
        </div>
    </div>
        ...
        $(document).ready(function() {
            ...
            $("#submitCommentButton").on("click", function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        alert("댓글 등록 실패: " + (xhr.responseJSON ? xhr.responseJSON.message : "서버 오류"));
                    }
                });
            });
            
            $("#comment-list-container").on("click", ".submit-reply-btn", function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        alert("대댓글 등록 실패: " + (xhr.responseJSON ? xhr.responseJSON.message : "서버 오류"));
                    }
                });
            });

            $("#comment-list-container").on("click", ".save-edit-btn", function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        alert("댓글 수정 실패: " + (xhr.responseJSON ? xhr.responseJSON.message : "서버 오류"));
                    }
                });
            });
            
            $("#comment-list-container").on("click", ".delete-comment-btn", function() {
            ...
            $("#comment-list-container").on("click", ".reply-comment-btn", function() {
                const commentId = $(this).data("comment-id");
                $('.reply-form').not('#reply-form-' + commentId).slideUp();
                $('#reply-form-' + commentId).slideToggle();
            });
            $("#comment-list-container").on("click", ".cancel-reply-btn", function() {
                $(this).closest('.reply-form').slideUp();
            });
            $("#comment-list-container").on("click", ".edit-comment-btn", function() {
                const commentId = $(this).data("comment-id");
            ...
            $("#comment-list-container").on("click", ".cancel-edit-btn", function() {
                const form = $(this).closest('.edit-form');
                form.slideUp(function() {
                    form.closest('.comment-item').find(".comment-content, .comment-actions").show();
                });
            });
        });
    </script>
