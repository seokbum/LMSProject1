...
<div class="container mt-5">
    ...
    <form id="postForm" action="/api/post/update" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <c:if test="${not empty post.existingFilePath}">
                <p>현재 파일: <span>${post.existingFilePath.substring(post.existingFilePath.lastIndexOf('_') + 1)}</span></p>
                <input type="hidden" name="existingFilePath" value="${post.existingFilePath}">
            </c:if>
        </div>
        <div class="mb-3">
            <label class="form-label">공지사항</label>
            <input type="checkbox" name="postNotice" id="postNotice" class="form-check-input" value="1" <c:if test="${post.postNotice == 1}">checked</c:if>>
